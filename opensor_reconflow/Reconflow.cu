...
sor::ReconFlow::ReconFlow() {
	this->BlockWidth = 32;
	this->BlockHeight = 12;
	this->StrideAlignment = 32;
}

sor::ReconFlow::ReconFlow(int BlockWidth, int BlockHeight, int StrideAlignment) {
...
int sor::ReconFlow::_initializeR(int width, int height, int channels, int nLevels, float scale, int method,
	float lambda = 100.0f, float lambdagrad = 400.0f, float lambdaf = 100.0f, float lambdams = 100.0f,
...
int sor::ReconFlow::copy3dToHost(cv::Mat &X, cv::Mat &Y, cv::Mat &Z) {
...
int sor::ReconFlow::setCameraMatrices(cv::Mat intrinsic) {
	//K0 = (double*)intrinsic.ptr();
	//K1 = (double*)intrinsic.ptr();
	return 0;
}

int sor::ReconFlow::setCameraMatrices(cv::Mat intrinsic0, cv::Mat intrinsic1) {
...
int sor::ReconFlow::solveR(float flowScale) {
	//Obsolete
	cv::Mat R1, t1;
	return solveReconFlow(R1, t1, flowScale);
}

int sor::ReconFlow::solveR() {
	//Obsolete
	cv::Mat R1, t1;
	return solveReconFlow(R1, t1, 5.0f);
}

int sor::ReconFlow::solveR(cv::Mat R1, cv::Mat t1, float flowScale) {
	return solveReconFlow(R1, t1, flowScale);
}

int sor::ReconFlow::solveR(cv::Mat R0, cv::Mat t0, cv::Mat R1, cv::Mat t1, float flowScale) {
	return solveReconFlow(R0, t0, R1, t1, flowScale);
}

int sor::ReconFlow::solveReconFlow(cv::Mat rot1, cv::Mat tr1,//input pose
...
int sor::ReconFlow::solveReconFlow(cv::Mat rot0, cv::Mat tr0, cv::Mat rot1, cv::Mat tr1,//input pose
	float flowScale) //display normalization
{
	cv::Mat Rt0, Rt1;
	...
	for (int level = nLevels - 1; level >= 0; level--) {
		...
		if ((method == METHODR_TVL1_MS_FN) || (method == METHODR_TVL1_MS_FNSPARSE) ||
			...
			if ((method == METHODR_TVL1_MS_FNSPARSE_LIDAR) || (method == METHODR_TVL1_MS_FN_LIDAR)) {
				DownscaleMask(d_spmask, pW[0], pH[0], pS[0], pW[level], pH[level], pS[level], d_spmask_l);
				DownscaleNearestNeighbor(d_Xsp, pW[0], pH[0], pS[0], pW[level], pH[level], pS[level], d_Xsp_l);
				DownscaleNearestNeighbor(d_Ysp, pW[0], pH[0], pS[0], pW[level], pH[level], pS[level], d_Ysp_l);
				DownscaleNearestNeighbor(d_Zsp, pW[0], pH[0], pS[0], pW[level], pH[level], pS[level], d_Zsp_l);
				/*cv::Mat maskView = cv::Mat(cv::Size(pS[level], pH[level]), CV_32F);
				checkCudaErrors(cudaMemcpy((float *)maskView.ptr(), d_Zsp_l, pS[level] * pH[level] * sizeof(float), cudaMemcpyDeviceToHost));
				cv::Mat view;
				maskView.convertTo(view, CV_8UC1, 10.0f);
				cv::imshow("mask", view);
				cv::imwrite("masktest" + std::to_string(level) + ".png", view);
				cv::waitKey();*/
			}
			DownscaleMask(d_fnmask, pW[0], pH[0], pS[0], pW[level], pH[level], pS[level], d_fnmask_l);
		...
		for (int warpIter = 0; warpIter < nWarpIters; warpIter++) {
			...
			//**************************************
			// TODO: COMPUTE AND DISPLAY TOTAL ERROR
			//**************************************
...
int sor::ReconFlow::removeEdgeArtifacts3D(int surfaceWidth, float maxSurfaceArea) {
