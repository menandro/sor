...
int sor::CudaFlow::initializeCorrelation(int kernelSize, int maxSearchWidth, int maxSearchHeight) {
...
int sor::CudaFlow::solveCorrPatchMatch(const char *picName, const char *flowName) {
	//this->_solveCorrPatchMatch();
	this->_solveCorrPatchMatch_cpu(picName, flowName);
	return 0;
}

int sor::CudaFlow::solveCorrPatchMatch() {
	//this->_solveCorrPatchMatch();
	this->_solveCorrPatchMatch_cpu("new", "new");
	return 0;
}

int sor::CudaFlow::_solveCorrPatchMatch_cpu(const char *picName, const char *flowName) {
	//bind textures
...
int sor::CudaFlow::_solveCorrPatchMatch() {
	//bind textures
	rgbToGray(d_i08uc3, d_icorr032f, width, height, stride);
	rgbToGray(d_i18uc3, d_icorr132f, width, height, stride);
	CorrelationBindTextures(d_icorr032f, d_icorr132f, width, height, stride);
...
int sor::CudaFlow::solveOpticalFlowLdof() {
	return this->_solveOpticalFlowLdof();
}

int sor::CudaFlow::_solveOpticalFlowLdof() {
	// Convert RGB to Gray
...
int sor::CudaFlow::copyCorrPatchMatchToHost(cv::Mat &u, cv::Mat &v, cv::Mat &uvrgb) {
	return 0;
}