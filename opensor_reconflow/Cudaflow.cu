...
//Constructors
sor::CudaFlow::CudaFlow() {
	this->BlockWidth = 32;
	this->BlockHeight = 12;
	this->StrideAlignment = 32;
}

sor::CudaFlow::CudaFlow(int BlockWidth, int BlockHeight, int StrideAlignment) {
...
int sor::CudaFlow::initialize(int width, int height, int channels, int cvType, int nLevels, float scale, int method,
	float lambda = 100.0f, float lambdagrad = 400.0f, float theta = 0.33f, float tau = 0.25f,
	int nWarpIters = 1, int nSolverIters = 100)
{
	bool withVisualization = true;
...
int sor::CudaFlow::initializeColorWheel() {
...
int sor::CudaFlow::initializeLargeDisplacement() {
...
int sor::CudaFlow::copyDepthToDevice(cv::Mat depth0, cv::Mat depth1) {
...
int sor::CudaFlow::copyImagesToDevice(cv::Mat i0, cv::Mat i1) {
	if (inputType == CV_8UC3) {
...
int sor::CudaFlow::copyOpticalFlowToHost(cv::Mat &u, cv::Mat &v, cv::Mat &uvrgb) {
...
int sor::CudaFlow::copyOpticalFlowToHost(cv::Mat &u, cv::Mat &v) {
	checkCudaErrors(cudaMemcpy((float *)u.ptr(), d_u, stride * height * sizeof(float), cudaMemcpyDeviceToHost));
	checkCudaErrors(cudaMemcpy((float *)v.ptr(), d_v, stride * height * sizeof(float), cudaMemcpyDeviceToHost));
	return 0;
}

int sor::CudaFlow::copyInitialOpticalFlowToDevice(cv::Mat u, cv::Mat v) {
...
int sor::CudaFlow::copySparseOpticalFlowToDevice(cv::Mat u, cv::Mat v) {
	cv::Mat mask = cv::Mat::ones(u.size(), CV_32F);
	copySparseOpticalFlowToDevice(u, v, mask);
	return 0;
}

int sor::CudaFlow::copySparseOpticalFlowToDevice(cv::Mat u, cv::Mat v, cv::Mat mask) {
	checkCudaErrors(cudaMemcpy(d_ufn, (float*)u.ptr(), dataSize32f, cudaMemcpyHostToDevice));
	checkCudaErrors(cudaMemcpy(d_vfn, (float*)v.ptr(), dataSize32f, cudaMemcpyHostToDevice));
...
int sor::CudaFlow::copySceneFlowToHost(cv::Mat &sceneflow) {
	checkCudaErrors(cudaMemcpy((float3 *)sceneflow.ptr(), d_sceneflow, dataSize32fc3, cudaMemcpyDeviceToHost));
	return 0;
}

int sor::CudaFlow::copySceneFlowToHost(cv::Mat &sceneflow, cv::Mat &sceneflowrgb) {
	checkCudaErrors(cudaMemcpy((float3 *)sceneflow.ptr(), d_sceneflow, dataSize32fc3, cudaMemcpyDeviceToHost));
	checkCudaErrors(cudaMemcpy((float3 *)sceneflowrgb.ptr(), d_sceneflowrgb, dataSize32fc3, cudaMemcpyDeviceToHost));
	return 0;
}

int sor::CudaFlow::copySceneFlowAndPointCloudToHost(cv::Mat &sceneflow, cv::Mat &pcloud0, cv::Mat &pcloud1, cv::Mat &sceneflowrgb) {
	checkCudaErrors(cudaMemcpy((float3 *)sceneflow.ptr(), d_sceneflow, dataSize32fc3, cudaMemcpyDeviceToHost));
...
int sor::CudaFlow::copyToDevice(cv::Mat i0, cv::Mat i1) {
	if (inputType == CV_8UC3) {
		checkCudaErrors(cudaMemcpy(d_i08uc3, (uchar3 *)i0.ptr(), dataSize8uc3, cudaMemcpyHostToDevice));
		checkCudaErrors(cudaMemcpy(d_i18uc3, (uchar3 *)i1.ptr(), dataSize8uc3, cudaMemcpyHostToDevice));
	}
...
//Solve Optical Flow
int sor::CudaFlow::solveOpticalFlow(float flowScale = 5.0f) {
	return _solveOpticalFlow(flowScale);
}

int sor::CudaFlow::solveOpticalFlow() {
	return _solveOpticalFlow(5.0f);
}

int sor::CudaFlow::_solveOpticalFlow(float flowScale) {
	// Convert RGB to Gray
	if (inputType == CV_8UC3) {
...
//OLD
int sor::CudaFlow::_solveOpticalFlow(cv::Mat i0, cv::Mat i1, cv::Mat &u, cv::Mat &v, cv::Mat &uvrgb, float flowScale) {
	// Convert RGB to Gray
...
int sor::CudaFlow::solveOpticalFlow(cv::Mat i0, cv::Mat i1, cv::Mat &u, cv::Mat &v, cv::Mat &uvrgb, float flowScale = 5.0f) {
	return _solveOpticalFlow(i0, i1, u, v, uvrgb, flowScale);
}
int sor::CudaFlow::solveOpticalFlow(cv::Mat i0, cv::Mat i1, cv::Mat &u, cv::Mat &v, cv::Mat &uvrgb) {
	return _solveOpticalFlow(i0, i1, u, v, uvrgb, 5.0f);
}


//Solve Scene Flow
int sor::CudaFlow::solveSceneFlow(float focalx, float focaly, float cx, float cy, float flowScale = 5.0f) {
...
int sor::CudaFlow::solveSceneFlow(float focalx, float focaly, float cx, float cy, float opticalFlowScale = 5.0f, float sceneFlowScale = 1.0f) {
	this->depthCameraFocalX = focalx;
	this->depthCameraFocalY = focaly;
	this->depthCameraPrincipalX = cx;
	this->depthCameraPrincipalY = cy;
...
int sor::CudaFlow::solveSceneFlow() {
	return _solveSceneFlow(5.0f, 1.0f);
}

int sor::CudaFlow::_solveSceneFlow(float opticalFlowScale, float sceneFlowScale) {
...
int sor::CudaFlow::solveSceneFlowAndPointCloud(float focalx, float focaly, float cx, float cy, float opticalFlowScale = 5.0f, float sceneFlowScale = 1.0f) {
	this->depthCameraFocalX = focalx;
	this->depthCameraFocalY = focaly;
	this->depthCameraPrincipalX = cx;
	this->depthCameraPrincipalY = cy;
...
int sor::CudaFlow::_solveSceneFlowAndPointCloud(float opticalFlowScale, float sceneFlowScale) {
	solveOpticalFlow(opticalFlowScale);
	//convert depths to 3d coordinates
	Cv16uTo32f(d_depth016u, d_depth032f, width, height, stride);
	Cv16uTo32f(d_depth116u, d_depth132f, width, height, stride);
...
int sor::CudaFlow::close() {

	return 0;
}
// Align up n to the nearest multiple of m
inline int sor::CudaFlow::iAlignUp(int n)
{
...
// swap two values
template<typename T>
inline void sor::CudaFlow::Swap(T &a, T &ax)
{
	T t = a;
	a = ax;
	ax = t;
...
//swap four values
template<typename T>
inline void sor::CudaFlow::Swap(T &a, T &ax, T &b, T &bx)
{
	Swap(a, ax);
	Swap(b, bx);
}
...
//swap eight values
template<typename T>
inline void sor::CudaFlow::Swap(T &a, T &ax, T &b, T &bx, T &c, T &cx, T &d, T &dx)
{
	Swap(a, ax);
	Swap(b, bx);
	Swap(c, cx);
...
int sor::CudaFlow::computePyramidLevels(int width, int height, int minWidth, float scale) {
	int nLevels = 1;
