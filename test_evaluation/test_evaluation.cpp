...
void getFilenames(Filenames *filenames, bool useStereo = true) {
	// Modify these
	filenames->itemNo0 = "0000000264";
	filenames->itemNo1 = "0000000263";
...
void getIvdataFilenames(Filenames *filenames) { // monocular stereo only
	// Modify these
	filenames->itemNo0 = "im20";
	filenames->itemNo1 = "im20";
...
int test_ivdata() {
	...
	cv::Mat depthOut8, depthGt8, depthOutJet, depthGtJet;
	depthOut = (depthOut / maxDepthDisplay)*256.0f;
	depthOut.convertTo(depthOut8, CV_8UC1);
	depthGt = (depthGt / maxDepthDisplay)*256.0f;
	depthGt.convertTo(depthGt8, CV_8UC1);
...
int test_kitti() {
	...
	for (int j = 0; j < height; j++) {
		for (int i = 0; i < stride; i++) {
			...
			float z = Z.at<float>(j, i);
			//buffer.push_back(cv::Vec3f(x, -y, -z));
			if (((z <= 200) && (z >= 4)) && ((x <= 100) && (y > -100)) && ((y <= 100) && (y > -100))) {
				buffer.push_back(cv::Vec3f(x, -y, -z));
			}
			else {
				x = std::numeric_limits<float>::quiet_NaN();
